...
class registre_gen extends dbForm {
	...
	// verifier le 2eme champ si $verifier = 1 dans gen/dyn/form.inc
		...
		if ($this->valF['finalite']==""){
			$this->msg= $this->msg.$imgv._('finalite')."&nbsp;"._('obligatoire').$f;
			$this->correct=False;
		}
	...
	function cleSecondaire($id,&$db,$val,$debug) {
		...
		$sql = "select * from destinataire where registre ='".$id."'";
		$res = $db->query($sql);
		if($debug==1) echo $sql;
		if (database::isError($res))
			...
			$nbligne=$res->numrows();
			$this->msg = $this->msg.$imgv._('il_y_a')." ".$nbligne." "._('destinataire')." "._('pour')." "._('registre')." [".$id."]<br>";
			if($nbligne>0)
				$this->correct=false;
		}
		// cle secondaire dossier
		$sql = "select * from dossier where registre ='".$id."'";
		$res = $db->query($sql);
		if($debug==1) echo $sql;
		if (database::isError($res))
			die($res->getMessage(). " => Echec  ".$sql);
			...
			$nbligne=$res->numrows();
			$this->msg = $this->msg.$imgv._('il_y_a')." ".$nbligne." "._('dossier')." "._('pour')." "._('registre')." [".$id."]<br>";
			if($nbligne>0)
				$this->correct=false;
		}
		// cle secondaire modificatif
		$sql = "select * from modificatif where registre ='".$id."'";
		$res = $db->query($sql);
		if($debug==1) echo $sql;
		if (database::isError($res))
			die($res->getMessage(). " => Echec  ".$sql);
			...
			$nbligne=$res->numrows();
			$this->msg = $this->msg.$imgv._('il_y_a')." ".$nbligne." "._('modificatif')." "._('pour')." "._('registre')." [".$id."]<br>";
			if($nbligne>0)
				$this->correct=false;
		}
	}// clesecondaire
